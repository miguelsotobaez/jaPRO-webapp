<?php
	...
	$db->query("DROP TABLE TeamAccounts");
	...
	if (!$db->query("CREATE TABLE IF NOT EXISTS Races(id INT UNSIGNED AUTO_INCREMENT, username VARCHAR(16), coursename VARCHAR(40), style TINYINT UNSIGNED, season TINYINT UNSIGNED, duration_ms INTEGER UNSIGNED, topspeed SMALLINT UNSIGNED, average SMALLINT UNSIGNED, end_time INTEGER UNSIGNED, rank SMALLINT UNSIGNED, entries SMALLINT UNSIGNED, season_rank SMALLINT UNSIGNED, season_entries SMALLINT UNSIGNED, PRIMARY KEY (id), CONSTRAINT user_course_style_season UNIQUE KEY (username, coursename, style, season))")) {
		echo $db->error;
	}
	...
	if (!$db->query("CREATE TABLE IF NOT EXISTS Teams(id SMALLINT UNSIGNED AUTO_INCREMENT, name VARCHAR(16), tag VARCHAR(16), longname VARCHAR(24), flags TINYINT UNSIGNED, PRIMARY KEY (id), UNIQUE KEY (name))")) {
		echo $db->error;
	}
	if (!$db->query("CREATE TABLE IF NOT EXISTS TeamAccounts(id SMALLINT UNSIGNED AUTO_INCREMENT, team VARCHAR(16), account VARCHAR(16), flags TINYINT UNSIGNED, PRIMARY KEY (id), UNIQUE KEY (team, account))")) {
		echo $db->error;
	}
