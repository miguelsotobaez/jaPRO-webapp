<?php
	...
	if (!$db->query("CREATE TABLE IF NOT EXISTS Accounts(id SMALLINT UNSIGNED AUTO_INCREMENT, username VARCHAR(16), kills SMALLINT UNSIGNED, deaths SMALLINT UNSIGNED, suicides SMALLINT UNSIGNED, captures SMALLINT UNSIGNED, returns SMALLINT UNSIGNED, lastlogin INT UNSIGNED, created INT UNSIGNED, lastip INT UNSIGNED, PRIMARY KEY (id), UNIQUE KEY (id))")) {
		echo $db->error;
	}
	if (!$db->query("CREATE TABLE IF NOT EXISTS Races(id INT UNSIGNED AUTO_INCREMENT, username VARCHAR(16), coursename VARCHAR(40), style TINYINT UNSIGNED, duration_ms INTEGER UNSIGNED, topspeed SMALLINT UNSIGNED, average SMALLINT UNSIGNED, end_time INTEGER UNSIGNED, rank SMALLINT UNSIGNED, entries SMALLINT UNSIGNED, last_update INTEGER UNSIGNED, PRIMARY KEY (id), UNIQUE KEY (id))")) {
		echo $db->error;
	}
	if (!$db->query("CREATE TABLE IF NOT EXISTS Duels(id INT UNSIGNED AUTO_INCREMENT, winner VARCHAR(16), loser VARCHAR(16), type TINYINT UNSIGNED, duration INTEGER UNSIGNED, winner_hp TINYINT UNSIGNED, winner_shield TINYINT UNSIGNED, end_time INTEGER UNSIGNED, winner_elo DECIMAL(6,2), loser_elo DECIMAL (6,2), odds DECIMAL (9,2), PRIMARY KEY (id), UNIQUE KEY (id))")) {
		echo $db->error;
	}
